...
results <- 
  ...
  mutate(ICL = ICL_choice == true_model, BIC  = BIC_choice == true_model, AIC = AIC_choice == true_model)
results %>% group_by(true_model, n) %>% 
  summarize(ICL_correct = mean(ICL), BIC_correct  = mean(BIC), AIC_correct = mean(AIC))
```
