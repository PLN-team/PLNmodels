...
Rcpp::List newton_optimize_full(
    ...
    return newton_optimize_impl<FullCovTraits>(Y, X, O, w, B, M, S2, state, cfg.maxiter, cfg.ftol, cfg.max_em, cfg.em_tol);
}
...
Rcpp::List newton_optimize_vestep_full(
    ...
    return newton_vestep_impl<FullCovTraits>(Y, X, O, w, M, S2, B, state, cfg.maxiter, cfg.ftol);
}
