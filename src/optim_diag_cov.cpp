...
Rcpp::List nlopt_optimize_newton_diagonal(
    ...
    // Initial omega2 from starting M, S
    arma::mat S2    = S % S;
    arma::rowvec sigma2 = (w.t() * (M % M + S2)) / w_bar;
    arma::rowvec omega2 = arma::pow(sigma2, -1);
    ...
    for (int em = 0; em < max_em; em++) {
        ...
        for (int it = 0; it < maxiter; it++) {
            ...
            A = arma::exp(Z + 0.5 * S2);
            {
                const arma::mat logS_cand = -0.5 * arma::log(A + ones_row * omega2);
        ...
        double elbo = arma::accu(w.t() * (Y % Z - A + 0.5 * arma::trunc_log(S2)))
                    - 0.5 * w_bar * arma::accu(arma::log(sigma2));
    ...
    arma::sp_mat Sigma_out(p, p); Sigma_out.diag() = sigma2.t();
    arma::sp_mat Omega_out(p, p); Omega_out.diag() = omega2_v;
    arma::mat Z = O + X * B + M;
    arma::mat A = arma::exp(Z + 0.5 * S2);
    arma::mat loglik = arma::sum(Y % Z - A + 0.5 * arma::log(S2), 1)
                     - 0.5 * (M % M + S2) * omega2_v
