...
double fn_optim_PLN_full(unsigned N, const double *x, double *grad, void *data) {
  ...
  double objective = accu((dat->w).t()*(A - dat->Y % Z - log(S))) - .5*dat->w_bar*real(log_det(Omega)) ;
...
double fn_optim_PLN_spherical(unsigned N, const double *x, double *grad, void *data) {
  ...
  double objective = accu(diagmat(dat->w) * (A - dat->Y % Z))  - p*accu(dat->w % log(S)) + .5*w_bar*p*log(sigma2) ;
...
double fn_optim_PLN_diagonal(unsigned N, const double *x, double *grad, void *data) {
  ...
  // objective function
  double objective = accu(diagmat(dat->w) *(A - dat->Y % Z - log(S)) ) + .5 * w_bar* accu(log(diag_Sigma)) ;
