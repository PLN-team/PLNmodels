...
// [[Rcpp::export]]
Rcpp::List nlopt_optimize_vestep_newton_spherical(
    const Rcpp::List & data  , // List(Y, X, O, w)
    ...
    for (int it = 0; it < maxiter; it++) {
        ...
        // ---- Fixed-point update for logS (scalar cov_diag) ----
    ...
    // ---- Final output ----
    S2 = S % S;
    const double sigma2 = 1.0 / omega2;
    arma::mat Z = O + XB + M;
    arma::mat A = arma::exp(Z + 0.5 * S2);
    arma::vec loglik = arma::sum(Y % Z - A - 0.5 * (M % M + S2) / sigma2 + 0.5 * arma::log(S2 / sigma2), 1)
                     + ki(Y);
