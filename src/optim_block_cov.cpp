...
Rcpp::List nlopt_optimize_block(
  ...
  auto objective_and_grad = [&metadata, &Y, &X, &O, &w, &w_bar, &Tau](const double * params, double * grad) -> double {
    ...
    metadata.map<M_ID>(grad) = diagmat(w) * (M * Omega + (A1 * Tau.t()) % A2 - Y * Tau.t());
  ...
  arma::mat mu = O + X * B;
  arma::mat Z = mu + M * Tau;
  arma::mat A = exp(mu) % ( exp(M + .5 * S2) * Tau) ;

  arma::vec loglik = sum(Y % Z - A, 1) + 0.5 * sum(log(S2), 1) - 0.5 * sum( (M * Omega) % M + S2 * diagmat(Omega), 1) +
  0.5 * real(log_det(Omega)) + ki(Y) + accu(arma::trunc_log(mean(Tau,1)).t() * Tau) + accu(Tau % arma::trunc_log(Tau)) ;
