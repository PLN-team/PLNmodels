...
double fn_optim_wPLN(const std::vector<double> &x, std::vector<double> &grad, void *data) {
  ...
  arma::mat Omega = n * inv_sympd(M.t() * diagmat(dat->w) * M + diagmat(dat->w.t()*S));
  arma::mat Z = dat->O + dat->X * Theta.t() + M;
  arma::mat A = exp (Z + .5 * S) ;

  double objective = accu(diagmat(dat->w) *(A - dat->Y % Z - .5*log(S)) ) - .5*accu(dat->w)*real(log_det(Omega)) + dat->KY ;

  arma::vec grd_Theta = vectorise((diagmat(dat->w)*(A-dat->Y) ).t() * dat->X);
  arma::vec grd_M     = vectorise(diagmat(dat->w) * (M * Omega + A-dat->Y)) ;
  arma::vec grd_S     = vectorise(.5 * (dat->w * diagvec(Omega).t() + diagmat(dat->w) * A - 1/(diagmat(dat->w)*S)));
