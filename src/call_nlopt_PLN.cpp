...
Rcpp::List optimization_PLN(
  ...
  nlopt::opt opt = initNLOPT((2 * n + d) * p, control) ;
  opt.set_xtol_abs(arma::conv_to<stdvec>::from(
    arma::join_cols(arma::zeros(p*(d+n)), Rcpp::as<double>(control["xtol_abs"]) * arma::ones(n * p))
  ));
  opt.set_lower_bounds(arma::conv_to<stdvec>::from(
    arma::join_cols(R_NegInf * arma::ones(p*(d+n)), Rcpp::as<double>(control["lbvar"]) * arma::ones(n * p))
  ));
