...
Data are originaly stored in a list of two data frames for abundances and covariates. We first prepare the data [see the corresponding vignette](import.html) for easy use in the multivariate framework of **PLNmodels**. Offsets are automatically computed:
...
select(trichoptera, -Offset, -Abundance) %>% head() %>% knitr::kable()
```
