...
```{r trichoptera, echo = FALSE, fig.cap = "Macronema Zebratum captured by Y. Dubuc at Donacona (Québec), 06-20-2001."}
...
Data are stored in the `trichoptera` data frame which includes 49 rows (the observations - or trapping nights) and 9 columns. As can be seen, there are 2 multivariate columns (matrices of counts and offsets) and 7 univariate columns (vectors of covariates):
...
A first glance at the pairs plot between the covariates shows no particular strong structure regarding the trichoptera groups.

```{r pairs, fig.dim = c(7, 7), cache = TRUE, fig.cap ="Pairs plot of the covariates in the trichoptera data set"}
select(trichoptera, -TotalCounts, -Abundance) %>% 
GGally::ggpairs(columns = 1:6, mapping = aes(colour = Group))
```
