...
ggplot(path, aes(x = Penalty, y = Coeff, group = Edge, colour = Edge)) + geom_line(show.legend = FALSE) + theme_bw()
```
...
If we want to select one network, we may extract best model according to BIC with the method `getBestModel("BIC")`. A model with a specific penalty level can be extracted with the `getModel(lambda)` method. 

```{r}
model.BIC <- models$getBestModel("BIC") # if no criteria is specified, the best BIC is used
model.pen <- models$getModel(models$penalties[5]) # give some sparsity
```

The variables `model.BIC` and `model.pen` are other `R6Class` objects of class `PLNnetworkfit` which in turns owns a couple of methods, mostly for vizualization purposes. The `plot_network` method provides a quick representation of the inferred network, with various options.
...
model.BIC$plot_network(type = "partial_cor", output = "igraph")
```
