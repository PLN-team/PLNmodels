...
```{r}
model.BIC <- models$getBestModel() # if no criteria is specified, the best ICL is used
model.pen <- models$getModel(models$penalties[5]) # give some sparsity
```

The variables `model.BIC` and `model.pen` are other `R6Class` objects of class `PLNnetworkfit` which in turns owns a couple of methods, mostly for vizualization purposes. The `plot_network` method provides a quick representation of the inferred network.
