...
You can project new data in the PCA sapce although it's slightly involved at the moment. We demonstrate that by projecting the original data on top of the original graph. As expected, the projections of the *new* data points (in red) are superimposed to the original data points (lightblue). 

```{r}
## Extract numeric matrices for model fitting
newdata <- trichoptera
...
## Compute latent variables (M)
newM <- myPCA_ICL$VEstep(covariates = covariates, offsets = offsets, 
                         responses = responses)$M
## Compute coordinates in PCA space
new_latent_pos <- t(myPCA_ICL$model_par$B %*% t(newM))
new_scores <- scale(new_latent_pos, TRUE, FALSE) %*% myPCA_ICL$rotation
dimnames(new_scores) <- list(rownames(newdata), paste0("PC", 1:ncol(new_scores)))
## Overprint
p <- factoextra::fviz_pca_ind(myPCA_ICL, geom = "point", col.ind = "lightblue")
