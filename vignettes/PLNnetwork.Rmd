...
2. a penalized log-likelihood step, performed with the graphical-Lasso of @FHT08, with an internal C++ port of the GLASSOFAST implementation [@glassofast], also available to users as `graphical_lasso()`.
...
Resampling can be computationally demanding but is easily parallelized: the function `stability_selection` relies on `parallel::mclapply` to perform parallel computing. Set the number of workers with the `mc.cores` option (forking-based, so only effective on Unix-like systems; ignored on Windows):
...
When you are done, do not forget to get back to the default (sequential) behavior.

```{r mc-cores-off, eval = FALSE}
options(mc.cores = 1)
```
