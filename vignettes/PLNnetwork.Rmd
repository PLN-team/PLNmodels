---
title: "Analyzing multivariate count table with sparse Poisson log-normal models"
...
This vignette intends to illustrate the basical use of the `PLNnetwork` function and the methods accompaning the R6 Classes `PLNnetworkfamily` and `PLNnetworkfit`.
...
In the package, the sparse PLN model is adjusted with the function `PLNnetowkr`, which we review in this section
...
```{r}
models <- PLNnetwork(Abundance ~ 1 + offset(log(TotalCounts)), data = trichoptera)
```
...
library(ggplot2)
ggplot(coefficient_path(models, corr = FALSE), aes(x = Penalty, y = Coeff, group = Edge, colour = Edge)) + 
...
ggplot(coefficient_path(models, corr = TRUE), aes(x = Penalty, y = Coeff, group = Edge, colour = Edge)) + 
    geom_line(show.legend = FALSE) + coord_trans(x = "log10") + theme_bw()
```
...
model.pen   <- getModel(models, models$penalties[20]) # give some sparsity
```
...
plot(model.BIC, type = "support", output = "corrplot")
```
...
Additional information about the optimization process comes with the option "diagnostic" in the `plot` method:
```{r diagnostic, fig.width=7, fig.height=5}
plot(models, "diagnostic")
```
