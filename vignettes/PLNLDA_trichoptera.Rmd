...
```{r predict_class}
set.seed(42)
ii <- sample(1:nrow(abundance), 10, replace = FALSE)
Y <- abundance[ii, ]
predicted.class <- myLDA_nocov$predict(newCounts = Y, 
                                       newdata = matrix(0, nrow(Y), 0),           ## No covariates
                                       newOffsets = matrix(0, nrow(Y), ncol(Y)),  ## No offset
                                       type = "response")
predicted.class
```
and asses that we recover the correct class in most cases:
```{r check_predicted_class}
table(predicted.class, night_grp[ii])
```
