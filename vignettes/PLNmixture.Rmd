...
```{r}
myMix_BIC$posteriorProb %>% head() %>% knitr::kable(digits = 2)
```
...
```{r vcov}
sigma(myMix_BIC) %>% purrr::map(diag)
```
...
The `PLNmixturefit` class also benefits from two important methods: `plot` and `predict`.
