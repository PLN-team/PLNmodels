...
\begin{equation}
  \mathbf{Z}_i | \mathbf{C}_i = k \, \sim \mathcal{N}({\mathbf{o}_i + \boldsymbol\mu}^{(k) + \mathbf{x}_i^\top{\boldsymbol\Theta}},{\boldsymbol\Sigma}^{(k)}),
\end{equation}
...
```{r cov, warning=FALSE}
mixtures_cov <- PLNmixture(Abundance ~ Wind + offset(log(Offset)), clusters = 1:4, data = trichoptera)
```

```{r plot cov, fig.width=7, fig.height=5}
plot(mixtures_cov, c("BIC", "loglik"))
```
