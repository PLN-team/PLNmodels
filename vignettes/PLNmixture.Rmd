...
data(mollusk)
trichoptera <- prepare_data(mollusk$Abundance, mollusk$Covariate)
```
...
\end{equation}
where each ${\boldsymbol\Theta}$ is a $d\times p$ matrix of regression parameters common to all the mixture components.
...
mixture_models <- PLNmixture(
  Abundance ~ 1 + offset(log(duration)),
  data  = trichoptera,
  clusters = 1:4
)
```
...
From this plot, we can see that the best model in terms of BIC is obtained for a number of clusters of `r which.max(mixture_models$criteria$BIC)`. We may extract the corresponding model with the method `getBestModel()`. A model with a specific number of clusters can be extracted with the `getModel()` method:
...
Let us extract the vector of group memberships:
```{r}
...
<!-- It is possible to correct for other covariates before performing the clustering. In our case ,we're going to use `Wind` as a covariate and illustrate the main differences with before  : -->
<!-- ```{r cov, warning=FALSE} -->
<!-- mixtures_cov <- PLNmixture(Abundance ~ 0 + site + offset(log(duration)), clusters = 1:4, data = trichoptera) -->
<!-- ``` -->

<!-- ```{r plot cov, fig.width=7, fig.height=5} -->
<!-- plot(mixtures_cov) -->
<!-- ``` -->

<!-- ```{r cov plot clustering} -->
<!-- myMix_BIC <- getBestModel(mixtures_cov, "BIC") -->
