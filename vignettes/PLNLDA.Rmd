...
- the $p \times K$ matrix of group means $\mathbf{M}$ (only 8 first groups heres)

```{r group-means}
myLDA_nocov$group_means[, 1:8] %>% head() %>% knitr::kable(digits = 2)
```
...
```{r extract-scores}
myLDA_nocov$scores[, 1:8] %>% head %>% knitr::kable(digits = 2)
```
