...
tibble(Label = 1:12, 
       ...
       Date  = str_c(rep(c("June", "July", "June", "July"), times = c(4, 1, 6, 1)), 
...
### A model with latent main effects and no covariates for the Trichopetra data set
...
```{r vcov}
vcov(myLDA_nocov) %>% corrplot::corrplot(is.corr = FALSE)
```
...
```{r group-means}
myLDA_nocov$group_means %>% head() %>% knitr::kable()
```
...
predicted.class %>% head() %>% knitr::kable()
```
...
Finally, we can assess that the predictions are quite similar to the real group (*this is not a proper validation of the method as we used dataset for both model fitting and prediction and are thus at risk of overfitting*). 
...
All fields of our new `PLNLDA` fit can be accessed as before with similar results. 

The only important difference is the result of `coef`: since we included a covariate in the model, `coef` now returns a 1-column matrix for $\hat{\boldsymbol{\Theta}}$ instead of `NULL`
...
```{r coef-cov}
myLDA_cov$group_means %>% head %>% knitr::kable()
```
