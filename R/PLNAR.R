...
#' @param data an optional data frame, list or environment (or object coercible by as.data.frame to a data frame) containing the variables in the model. If not found in data, the variables are taken from environment(formula), typically the environment from which PLNAR is called.
#' @param subset an optional vector specifying a subset of observations to be used in the fitting process.
#' @param weights an optional vector of observation weights to be used in the fitting process.
#' @param control a list-like structure for controlling the optimization, with default generated by [PLNAR_param()]. See the associated documentation
#' for details.
#'
#' @return an R6 object with class [`PLNARfit`]
#'
#' @rdname PLNAR
...
#' myPLNAR <- PLNAR(Abundance ~ 1, data = trichoptera)
#' @seealso The class [`PLNARfit`] and the configuration function [PLNAR_param()]
#' @importFrom stats model.frame model.matrix model.response model.offset model.weights terms
#' @export
PLNAR <- function(formula, data, subset, weights, control = PLNAR_param()) {

  ## Temporary test for deprecated use of list()
  if (!inherits(control, "PLNARmodels_param"))
    stop("We now use the function PLNAR_param() to generate the list of parameters that controls the fit:
    replace 'list(my_arg = xx)' by PLNAR_param(my_arg = xx) and see the documentation of PLNAR_param().")
  ...
  myPLNAR <- switch(control$covariance,
      "diagonal"  = PLNARfit_diagonal$new(args$Y, args$X, args$O, args$w, args$formula, control),
      "spherical" = PLNARfit_spherical$new(args$Y, args$X, args$O, args$w, args$formula, control),
      "fixed"     = PLNARfit_fixedcov$new(args$Y, args$X, args$O, args$w, args$formula, control),
      # "genet"     = PLNARfit_$new(args$Y, args$X, args$O, args$w, args$formula, control),
                    PLNARfit$new(args$Y, args$X, args$O, args$w, args$formula, control)) # default: full covariance

  ## optimization
  if (control$trace > 0) cat("\n Adjusting a", myPLNAR$vcov_model, "covariance PLNAR model with", control$backend, "optimizer")
  myPLNAR$optimize(args$Y, args$X, args$O, args$w, control$config_optim)

  ## post-treatment
  if (control$trace > 0) cat("\n Post-treatments...")
  myPLNAR$postTreatment(args$Y, args$X, args$O, args$w, control$config_post)

  if (control$trace > 0) cat("\n DONE!\n")
  myPLNAR
}

#' Control of a PLNAR fit
#'
#' Helper to define list of parameters to control the PLNAR fit. All arguments have defaults.
#'
...
#'    log-transformed data, and with the same formula as the one provided by the user. However, the user can provide a PLNARfit (typically obtained from a previous fit),
...
PLNAR_param <- function(
    ...
    config_post   = list(),
    config_optim  = list(),
    inception     = NULL     # pretrained PLNARfit used as initialization
) {

  covariance <- match.arg(covariance)
  if (covariance == "fixed") stopifnot(inherits(Omega, "matrix") | inherits(Omega, "Matrix"))
  if (!is.null(inception)) stopifnot(isPLNARfit(inception))

  ## post-treatment config
  config_pst <- config_post_default_PLNAR
  ...
  structure(list(
    ...
    inception     = inception), class = "PLNARmodels_param")
}
