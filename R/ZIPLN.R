...
#'
#' Fit the multivariate Zero Inflated Poisson lognormal model with a variational algorithm. Use the (g)lm syntax for model specification (covariates, offsets, weights).
#'
...
#' @export
ZIPLN <- function(formula, data, subset, weights, control = list()) {
  ...
  ## initialization
  if (ctrl$trace > 0) cat("\n Initialization...")
  myPLN <- ZIPLNfit$new(args$Y, args$X, args$O, args$w, args$formula, args$xlevels, ctrl)
