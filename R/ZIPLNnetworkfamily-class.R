...
ZIPLNnetworkfamily <- R6Class(
  ...
  public = list(
    ...
    initialize = function(penalties, responses, covariates, offsets, weights, formula, control) {
      ...
      super$initialize(responses, covariates$PLN, offsets, weights, control)
...
### TODO check if it is useful
    ...
    optimize = function(config) {
      ...
      for (m in seq_along(self$models))  {
        ...
        self$models[[m]]$optimize(self$responses, list(PLN = self$covariates, ZI = self$covariates_ZI), self$offsets, self$weights, config)
